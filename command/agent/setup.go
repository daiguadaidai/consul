...
func newBaseDeps(cmd *cmd, out io.Writer) (agent.BaseDeps, error) {
	...
	cfg.NodeID, err = setupNodeID(cfg, d.Logger)
	if err != nil {
	...
	acConf := new(autoconf.Config).
		WithDirectRPC(d.ConnPool).
		WithLogger(d.Logger).
		WithCertMonitor(acCertMon)
	acConf.Loader = func(src config.Source) (*config.RuntimeConfig, []string, error) {
		return config.Load(cmd.flagArgs, src)
	}
